...
- **Admin console:** Overview shows arq job breakdown, Redis **throttle** key PTTLs (including `ggg_trade2_lock`), price-job samples with **Updated** (UTC) from last `save_job_state`, and optional **manual** or **user-started** live refresh to `/admin/api/summary` (no auto-poll on login when live refresh is enabled). **Users** page adds headline adoption stats and Chart.js time series (signups, activity events, gear history, pricing/shares). See [admin/README.md](admin/README.md).
...
- `0008_character_shares` — public character share links
- `0009_user_activity_events` — append-only login/refresh events for admin Users charts
...
- **GGG weight group (trade2)**: GGG's server-side trade2 API rejects `weight`-type stat groups from unauthenticated (non-browser-session) callers with HTTP 400 "Query is too complex". The `weight` group has a high base complexity that exceeds the anonymous budget regardless of filter count — `and` groups with 6 filters (complexity 14) succeed while `weight` groups with even 3 filters fail. `submit_trade_search` now returns a 4-tuple `(search_id, data, rate_limited, status_code)`; callers check `status_code == 400` to distinguish this from other errors. The `weighted_upgrade` handler in `trade.py` automatically retries as a regular `upgrade` search on 400. **Note:** the trade2 API actively rejects GGG OAuth Bearer tokens with `insufficient_scope` (confirmed 2026-09-15) — it is not part of the OAuth program. There is no authenticated server-side path for the complexity budget issue. See `docs/ggg_api_compliance.md`.
- **GGG trade2 403 on datacenter IPs**: GGG/Cloudflare returns HTTP 403 (code 6 "Forbidden") for anonymous server-side trade2 API calls from datacenter (e.g. DigitalOcean) IPs. **Confirmed 2026-09-15:** the trade2 API actively rejects OAuth Bearer tokens with `{"error": "insufficient_scope"}` — the trade2 endpoint is scope-gated outside the OAuth program entirely. No fix via token forwarding is possible. The 403 on datacenter IPs is a **known limitation** for PROD. See [`docs/ggg_api_compliance.md`](docs/ggg_api_compliance.md) for the full analysis and options (residential proxy, contacting GGG, or accepting the limitation). UAT (home IP) is unaffected.
